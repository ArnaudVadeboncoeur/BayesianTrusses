...
/*Modified from MCMC.hpp -> -Burn in and thinning done in separate method
 *                          -Repeat sample if rejection occurs
 *
 */
#ifndef MCMC_HPP_
#define MCMC_HPP_
...
public:
	...
	void thinSamples ( unsigned burnIn, unsigned keepSamp );
...
void MCMC<DIM,FUNC,ARG>::sample( unsigned nSamples ){
    ...
    for( ; acceptedCandidates_.size() < nSamples && totalPoints < maxPoints; totalPoints++ ){
          ...
          acceptedCandidates_.push_back( std::make_pair( x_, yCurr ) );
	...
	try{
		...
		acceptanceRatio_ =  ( acceptedCandidates_.size() - rejectedNum_ ) / (double) acceptedCandidates_.size() ;

		//std::cout << "this line "<<acceptedCandidates_.size() << "  "<< rejectedNum_ << "  "
		//          << acceptedCandidates_.size() - rejectedNum_<<"  "  << acceptanceRatio_ << std::endl;
...
template<unsigned DIM, typename FUNC, typename ARG>
void MCMC<DIM, FUNC, ARG>::thinSamples ( unsigned burnIn, unsigned keepSamp ){
